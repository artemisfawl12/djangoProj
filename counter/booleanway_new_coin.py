...
from counter.coin_data_collect_bitthumb import request_data_byminute, request_data_bydate
import pandas as pd
...
def prepare_data(df):
    #dataframe을 받아서, ticker을 key, value는 index가 시간이고 나머지가 column인 dictionary??로 만들어서 준다?? 이게 말이되노
    grouped_data = {}
...
def trade_multiple(start_date, end_date, tickers, exec_code):
    ...
    for ticker in tickers:
        ...
        totalmonitordf_final = pd.concat(total_monitoring_list, keys=ticker_list, names=["tickers", "시간"])
        multi_ret_list = []
...
def trade(start_date, end_date, ticker,exec_code,unit, startmoney=100000000, chart_draw_ornot=1):
    loggers.info("trade func in booleanway_new_coin.py started")
